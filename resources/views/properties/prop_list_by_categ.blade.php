@extends('layouts.app')
    @section('content')
            ...
            <section class="latest-list">
                ...
                ?>
                <h1><i class="fa fa-th"></i> {!! $categType !!}</h1>
                <ul class="list-ul">
                    <?php
                    ...
                    foreach ($categs as $categ) {
                        ...
                        if($categ->rel_bed == 0){
                            ...
                            $title = $categ->rel_bed . ' bedroom';
                            $specs = '<article id="specs">
                                    <i class="fa fa-bed"></i> ' . $categ->rel_bed . '
                                    <span id="sep-h">|</span>
                                    <i class="re re-bathroom"></i> ' . $categ->rel_bath . '
                                </article>';
                        }
                    


                        $categ_link = explode(' ', $category_arr[$categ->rel_category]);
                        $categ_link = implode('-', $categ_link);

                        $status_link = explode(' ', $status_arr[$categ->rel_status_type]);
                        ...
                        echo '
                            <section id="photo" style="background: url(photos/thumbs/' . $categ->rel_photo_rep . ') no-repeat bottom center; background-size: cover;"></section>

                            <article id="title">
                                <a href="' . url('/properties/' . $categ_link .'/'. $status_link . '/' . $categ->rel_ref_id) . '">' . $title . ' ' . $category_arr[$categ->rel_category] . ' ' . $status_arr[$categ->rel_status_type] . '</a>
                            </article>

                            <article id="loc">
                                <i class="fa fa-map-marker"></i> ' . $categ->rel_address . ', ' . $categ->rel_town . '
                            </article>
                            
                            ' . $specs . '
                            
                            <article id="price">';
                                echo ($categ->rel_curr == 1) ? 'GH&cent;' : '$';
                                echo number_format($categ->rel_price, 0);
                                echo ($categ->rel_status_type == 2)? "<span style=\"font-size:15px;\">/mon</span>" : "";
                                echo '
                            </article>
                            
                            <article id="view-details">
                                <a href="' . url('/properties/' . $categ_link .'/'. $status_link . '/' . $categ->rel_ref_id) . '">view details</a>
                            </article>
                        </li>';
                ...
                <center style="display: block; margin-top: 50px;">
                    ...
                    <strong>0</strong> results for <strong>{!! $categType !!}</strong>
                    <p>
